...
| **Kiro** (CLI/IDE) | manual `bun <plugin>/hooks/compose.ts` after the folder-drop (see §9 — the `.kiro.hook` auto-fire and an `aidlc plugin compose` CLI are not yet wired) | n/a — folder-drop distribution |
