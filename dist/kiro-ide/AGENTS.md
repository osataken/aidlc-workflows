...
- **Activation**: Open the project in Kiro IDE. The install selects the `aidlc` conductor as the workspace default and registers `.kiro/hooks/*.kiro.hook` in the IDE's Agent Hooks panel.
...
- **Agents**: `.kiro/agents/` — 14 agents: 11 domain-expert personas (product, design, delivery, architect, aws-platform, compliance, devsecops, developer, quality, pipeline-deploy, operations), 2 review-only agents (product-lead, architecture-reviewer), and the adaptive-workflows composer. On Kiro the conductor is `agents/aidlc.json`; dedicated JSON configs cover the two stage workers, two reviewers, and composer that the conductor delegates through the Kiro `subagent` tool, while domain personas can also be adopted inline.
