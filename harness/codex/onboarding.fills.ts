...
- **Personal overrides**: Settings in \`~/.codex/config.toml\` merge over the project \`.codex/config.toml\`. Put machine-specific overrides (model, AWS profile/region, environment variables) there to avoid changing the shared project config.`,
    ...
    agents_note: `On Codex the agent personas are transposed into \`.agents/\` TOMLs (the conductor reads the persona \`.md\` bodies as prose); the dispatched stages (2.1 pipeline, 3.5 subagent) and ensemble collaborators run as spawned agent roles.`,
